...
fn final_scene(image_width: usize, samples_per_pixel: usize, max_depth: usize) {
    ...
    let shpere_material = Arc::new(Lambertian::new(Color::new(0.7, 0.3, 0.1)));
    world.add(Arc::new(Sphere::new_moving(
        center1,
        center2,
        50.0,
    ...
    let pertext = Arc::new(NoiseTexture::new(&mut rng, 0.2));
    world.add(Arc::new(Sphere::new_stationary(
    ...
    for _ in 0..ns {
        boxes2.add(Arc::new(Sphere::new_stationary(
            Point3::random(&mut rng, 0.0, 165.0),
            10.0,
            white.clone(),
        )));
    }
    ...
    let aspect_ratio = 1.0;
    let image_width: usize = image_width;
